    ...
    <div className='fixed inset-0 z-50 flex items-center justify-center'>
      ...
      <div className='relative bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto'>
        ...
        <div className='sticky bottom-0 bg-gray-50 border-t border-gray-200 px-6 py-4 flex justify-end gap-3'>
          <button
            onClick={onClose}
            className='px-6 py-2.5 rounded-lg border border-gray-300 text-[#1A1A1A] text-sm font-medium font-["Poppins"] hover:bg-gray-100 transition-colors'
          >
            {t('dashboard.admin.customerManagement.modal.close')}
          </button>
          <button
            onClick={handleAction}
            className={`px-6 py-2.5 rounded-lg text-white text-sm font-medium font-["Poppins"] transition-colors ${
              customer.status === 'active'
