...
const Pagination = ({
  ...
  const PrevNext = ({
    btnClass = 'px-3 py-1.5 text-sm text-gray-700 border border-gray-300 hover:bg-gray-50 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium',
    wrapClass = 'flex flex-row flex-wrap items-center gap-2',
    pagesUI,
  }) => (
  ...
  const Layout = ({ children }) => (
    <div className={`${className}`}>
      <div className='flex flex-col sm:flex-row justify-between items-center gap-2 border-t border-gray-200 px-4 md:px-6 py-3 md:py-4'>
        {showCount ? (
          <p className='text-sm text-gray-600'>
            {showCount.prefix || 'Showing'} {showCount.current}{' '}
            {showCount.label || 'of'} {showCount.total}
          </p>
        ) : (
